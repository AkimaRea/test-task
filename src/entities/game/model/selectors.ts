...
export const selectVisibleGames = createSelector(
  [selectAllGames, selectProviderFilter],
  (games, filter) =>
    games.filter((g) => g.isActive && (!filter || g.provider === filter))
);

export const selectAdminGames = createSelector(
  [selectAllGames, selectProviderFilter],
  (games, filter) =>
...
export const selectProviderOptions = createSelector(
  [selectProviders],
  (providers): SelectOption[] => providers.map((p) => ({ value: p, label: p }))
);
...
export const selectProviderFilterOption = createSelector(
  ...
  (filter): SelectOption =>
    filter ? { value: filter, label: filter } : { value: "", label: "" }
);
